...
WHISPER_WORKERS ?= 3
transcribe: ## Transcribe videos from cast JSON — usage: make transcribe CAST=cast.json [WHISPER_WORKERS=3]
	@python3 scripts/transcribe.py --cast $(CAST) --whisper http://localhost:$(WHISPER_PORT) --workers $(WHISPER_WORKERS)
...
pipeline: ## ONE CMD — full pipeline: cast→transcript→context→index — usage: make pipeline CAST=cast.json [CHUNKS=8] [WORKERS=24] [WHISPER_WORKERS=3]
